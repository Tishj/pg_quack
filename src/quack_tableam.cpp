...
quack_begin_scan(Relation rel, Snapshot snapshot, int nkeys,
...
static void
quack_rescan(TableScanDesc scan, struct ScanKeyData * key,
...
static bool
quack_getnextslot(TableScanDesc scan, ScanDirection direction,
                  TupleTableSlot * slot)
{
    return false;
}
...
static bool
quack_index_fetch_tuple(struct IndexFetchTableData * sscan,
                       ItemPointer tid,
                       Snapshot snapshot,
                       TupleTableSlot * slot,
...
static bool
quack_fetch_row_version(Relation relation,
                        ItemPointer tid,
                        Snapshot snapshot,
                        TupleTableSlot *slot)
{
...
static bool
quack_tuple_satisfies_snapshot(Relation rel, TupleTableSlot * slot,
                               Snapshot snapshot)
{
...
quack_index_delete_tuples(Relation rel,
...
static void
quack_tuple_insert(Relation rel, TupleTableSlot * slot, CommandId cid,
                   int options, struct BulkInsertStateData * bistate)
{
  QuackWriteState * write_state = 
    quack_init_write_state(rel, MyDatabaseId, GetCurrentSubTransactionId());
  ...
  for (int i = 0; i < slot->tts_nvalid; i++)
  {
    ...
    {
      quack_append_value(write_state->appender, 
...
static void 
quack_tuple_insert_speculative(Relation rel, TupleTableSlot * slot,
                               CommandId cid, int options,
                               struct BulkInsertStateData * bistate,
                               uint32 specToken)
{
   ereport(ERROR,
           (errmsg("quack_tuple_insert_speculative is not implemented")));
}


static void
quack_tuple_complete_speculative(Relation rel, TupleTableSlot * slot,
                                 uint32 specToken, bool succeeded)
{
  ereport(ERROR,
          (errmsg("quack_tuple_complete_speculative is not implemented")));
}

static void
quack_multi_insert(Relation rel, TupleTableSlot ** slots, int ntuples,
                   CommandId cid, int options,
                   struct BulkInsertStateData * bistate)
{
 QuackWriteState * write_state = 
...
static TM_Result 
quack_tuple_delete(Relation rel, ItemPointer tip, CommandId cid,
...
static TM_Result
quack_tuple_update(Relation rel, ItemPointer tip,
...
static TM_Result
quack_tuple_lock(Relation rel, ItemPointer tid, Snapshot snapshot,
                 TupleTableSlot * slot, CommandId cid,
...
static void
quack_relation_set_new_filenode(Relation rel,
...
static void
quack_relation_copy_data(Relation rel, const RelFileNode * newrnode)
{
...
static void
quack_relation_copy_for_cluster(Relation OldHeap, Relation NewHeap,
...
static void
quack_vacuum_rel(Relation rel, VacuumParams * params,
...
static bool
quack_scan_analyze_next_block(TableScanDesc scan, BlockNumber blockno,
                              BufferAccessStrategy bstrategy)
{
...
static bool
quack_scan_analyze_next_tuple(TableScanDesc scan, TransactionId OldestXmin,
...
static double
quack_index_build_range_scan(Relation columnarRelation,
...
static void
quack_index_validate_scan(Relation columnarRelation,
                          Relation indexRelation,
                          IndexInfo * indexInfo,
                          Snapshot snapshot,
...
static void
quack_estimate_rel_size(Relation rel, int32 * attr_widths,
...
static bool
quack_scan_sample_next_tuple(TableScanDesc scan, SampleScanState * scanstate,
                             TupleTableSlot * slot)
{
...
static void
TableAMObjectAccessHook(ObjectAccessType access, Oid classId, Oid objectId,
                        int subId, void * arg)
{
  ...
  if (access == OAT_DROP && classId == RelationRelationId && !OidIsValid(subId))
  {
    ...
    if (rel->rd_rel->relkind == RELKIND_RELATION &&
        rel->rd_tableam == &quack_am_methods)
    {
